...
│                                                                 │
│  ┌─────────────────────┐  ┌─────────────────────┐              │
│  │ workpaw-web (Nginx) │  │ workpaw-admin/console       │              │
│  │ 用户配置应用         │  │ (Nginx) 管理后台    │              │
│  └─────────────────────┘  └─────────────────────┘              │
│                                                                 │
│  ┌─────────────────────┐                                       │
...
| workpaw-admin/console | 管理后台 | React + Vite + Shadcn UI | K8s (Nginx) |
| workpaw-admin | Control Plane API | Go + Gin + Zap + Viper | K8s (Pod) |
...
### 7.3 管理后台（workpaw-admin/console）
...
workpaw-admin/console   ──→ 引用 workpaw-ui
```
...
├── workpaw-admin: 骨架 + OIDC + JWT
└── workpaw-ui: 骨架 + 类型 + API 客户端

Week 3-4: 联调 + 前端
├── workpaw-admin: 实例管理 API

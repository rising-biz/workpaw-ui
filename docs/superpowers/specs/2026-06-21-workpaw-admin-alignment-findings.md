# workpaw-control-plane/console 对齐点核查结果

> 日期：2026-06-21
> 对应 spec：`2026-06-21-workpaw-control-plane/console-design.md` §12（实现前对齐点）
...
### 文件读取
- `workpaw-control-plane/internal/service/instance.go`（第 240–273 行，`GetConnectInfo`）
...
- `workpaw-control-plane/internal/service/instance.go`（`ActivateInstance` 第 186–209 行）
...
- `workpaw-control-plane/internal/service/instance.go`（`mapInstanceStatus` 第 280–305 行）
...
### 文件读取
- `workpaw-control-plane/internal/service/jwt.go`（现有 HS256 实现）
- `workpaw-control-plane/internal/config/`（配置结构，间接）
...
### 文件读取
- `workpaw-control-plane/internal/service/oidc.go`（`NewOIDCService` 第 52–78 行）
...
| `workpaw-control-plane/internal/service/instance.go` | 2, 4, 5 |
| `workpaw-operator/api/v1alpha1/qwenpawinstance_types.go` | 4, 5 |
| `workpaw-control-plane/internal/service/jwt.go` | 6 |
| `workpaw-control-plane/internal/service/oidc.go` | 7 |

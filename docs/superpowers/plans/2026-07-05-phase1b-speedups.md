...
**Files:**
- Modify: `workpaw-control-plane/internal/service/instance.go`（struct `:25-32`、`NewInstanceService` `:104-141`）
...
```go
import "github.com/workpaw/workpaw-control-plane/internal/config"
...
- Modify/Create: `workpaw-control-plane/internal/service/config_reconciler_test.go`
...
**Files:**
- Create: `workpaw-control-plane/internal/service/config_sync_controller.go`
- Create: `workpaw-control-plane/internal/service/config_sync_controller_test.go`
...
func StartConfigSyncController(ctx context.Context, cfg *config.Config, instanceSvc *InstanceService, rec *ConfigReconciler, logger *zap.Logger) error {
	...
	opts := ctrl.Options{
		...
		LeaderElectionID: "workpaw-control-plane-config-sync.leader",
	}
...
**Files:**
- Modify: `workpaw-control-plane/internal/router/router.go`（ConfigReconciler 块 `:293-307`）
- Modify: `workpaw-control-plane/cmd/serve.go`
...
cd /Users/zhangsan/workpaw/workpaw-control-plane && go run . serve --dev
...
**Files:**
- Modify: `workpaw-control-plane/internal/handler/auth.go`（struct `:19-30`、Callback `:115-225`）
- Modify: `workpaw-control-plane/internal/router/router.go`（注入 precreator）
- Create: `workpaw-control-plane/internal/handler/auth_precreate_test.go`
...
```go
import "github.com/workpaw/workpaw-control-plane/internal/service"
...
- control-plane workpaw-control-plane/feature/phase1b-speedups (main..<HEAD>): N commits

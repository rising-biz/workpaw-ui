...
| workpaw-admin/console | `.env.development` | control-plane URL | 新建/改 |
| workpaw-admin | `config.yaml` | 核对 K8s 配置（预期无需改） | 核对 |
...
**Files:** 核对 `workpaw-admin/config.yaml`（预期无改动）
...
Run: `cat workpaw-admin/config.yaml`
Expected 包含且与集群一致：
- `server.mode: debug`
...
```
cd workpaw-admin && git add config.yaml && git commit -m "chore: ensure debug mode for dev-login in dev config"
```
若全部一致，无提交。
...
Run（新终端）: `cd workpaw-admin && go run . serve --dev`
Run（另一终端）:
```
...
## Task 5: workpaw-admin/console 配置 control-plane URL

**Files:**
- Create/Modify: `workpaw-admin/console/.env.development`
...
若不存在则创建 `workpaw-admin/console/.env.development`，内容：
```
VITE_CONTROL_PLANE_URL=http://localhost:8090
```
...
Run:
```
cd workpaw-admin/console && npx tsc -b --noEmit
cd workpaw-admin/console && npm test
```
Expected: 无错误，测试通过（`.env` 改动不影响现有测试）。

- [ ] **Step 3: 手动验证**

确保 Task 1 完成（control-plane 运行）。Run: `cd workpaw-admin/console && npm run dev`
...
```
cd workpaw-admin/console && git add -A && git commit -m "chore: pin control-plane URL for dev against K8s cluster"
```
...
Run: `cd workpaw-admin/console && npm run dev`，dev-login → 管理页面加载。
...
- 在 `workpaw-admin/internal/config/config.go` 的 `IngressConfig` 增加 `Port int` 与 `Scheme string` 字段（mapstructure `port`/`scheme`），`Load()` 设默认 `Scheme:"https"`、`Port:0`。
...
- `cd workpaw-admin && go build ./... && go run . serve --dev` 验证 connect 返回的 URL 带正确端口/协议。
- 提交：`cd workpaw-admin && git commit -m "feat: configurable ingress scheme/port for non-443 dev clusters"`。
